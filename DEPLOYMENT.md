...
5. **Media persistence:** serverless/container filesystems are ephemeral, and uploads default to `public/media` on disk. `@payloadcms/storage-s3` is already wired in (`src/plugins`) and activates automatically when the five `S3_*` variables below are set — otherwise media falls back to local disk (fine for local dev, not for Vercel/Fly). Configure Supabase Storage:
   ...
   - **Create the bucket before the first seed.** `POST /next/seed` uploads the hero image as its first real step, so seeding a project whose bucket does not exist fails with `NoSuchBucket: Bucket not found`. The name is matched exactly and is case-sensitive, and `S3_ENDPOINT` must carry the `<project-ref>` of the project the bucket actually lives in — a bucket created in a different Supabase project produces the same error even though the credentials authenticate fine.
